...
class ReporteTipoDocumentoController extends Controller
{
    ...
    {
        $tipos = TipoDocumento::orderBy('nombre_tipoDocumento')
        ->get();

        return view('reporte.tipo-documento.index', compact('tipos'));
    }

    public function imprimir($fecha_inicio, $fecha_fin, $tipo_tramite, $tipoDocumento_id)
    {
       ///dd($Reportes);
      ...
      INNER JOIN tipodocumento td on td.id=d.tipoDocumento_id where d.fecha_recepcion between '$fecha_inicio' and '$fecha_fin' and d.tipo_tramite='$tipo_tramite' and d.tipoDocumento_id=$tipoDocumento_id");

   // dd($Reportes);
        
        $view = view('reporte.imprimir',['Reportes' => $Reportes]);
        $pdf = App::make('dompdf.wrapper');
        $pdf->setPaper('A4', 'landscape');

        $pdf->loadHTML($view);
        
       return $pdf->download('reporte_Tipodocumentos.pdf');
    }
    }